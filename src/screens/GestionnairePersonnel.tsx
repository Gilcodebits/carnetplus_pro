...
export function GestionnairePersonnel() {
  ...
  return (
    <div className="h-full bg-slate-50/50 p-4 md:p-8 flex flex-col gap-6 md:gap-8 overflow-hidden">
      
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
        <div>
          <h1 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight">Gestion de <span className="text-blue-600">l'Équipe</span></h1>
          <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest mt-1 flex items-center gap-2 opacity-100">
        ...
        <div className="overflow-auto flex-1">
          <table className="w-full text-left border-collapse relative">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-100/95 backdrop-blur-md border-b border-slate-200">
                <th className="px-8 py-5 text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Membre</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Fonction</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Contact</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Statut</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] text-right">Actions</th>
              </tr>
            </thead>
                  ...
                  <motion.tr key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="hover:bg-blue-50/40 transition-colors group">
                    <td className="px-8 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-slate-900 mt-0.5 opacity-60">ID #{u.id}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-4">
                    ...
                    <td className="px-8 py-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-bold text-slate-900 opacity-60">{u.telephone || 'Non renseigné'}</span>
                      </div>
                    </td>
                    <td className="px-8 py-4">
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[100] p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg border border-slate-200 overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                <div className="pt-4 flex gap-3">
                  <button type="submit" disabled={submitting} className="flex-1 py-4 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 shadow-lg disabled:opacity-50 active:scale-95">{submitting ? '...' : 'CONFIRMER'}</button>
                  <button type="button" onClick={() => setShowModal(false)} className="px-8 py-4 bg-white text-slate-900 border-2 border-slate-900 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all active:scale-95">ANNULER</button>
                </div>
              </form>
            </motion.div>
          </div>
        ...
        {showConfirmDelete && (
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl text-center border border-slate-100">
              ...
              <p className="text-sm font-bold text-slate-900 opacity-60 mb-8">
