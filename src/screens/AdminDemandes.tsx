...
export function AdminDemandes() {
  ...
  return (
    <div className="animate-fadeIn bg-slate-50 min-h-screen relative w-full max-w-full overflow-x-hidden flex flex-col">
      {/* Status Toasts */}
      <AnimatePresence>
        {statusMessage && (
          <motion.div
            ...
            className={`fixed top-10 left-1/2 z-[110] p-6 rounded-[2rem] shadow-2xl border-2 flex items-center gap-6 min-w-[400px] backdrop-blur-xl ${statusMessage.type === 'success'
          ...
          <div className="flex gap-2 p-1.5 bg-slate-50 rounded-2xl border-2 border-slate-100 w-full lg:w-auto">
            {["en_attente", "approuve", "rejete", "tous"].map(f => (
              <button
                ...
                className={`flex-1 lg:px-8 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeFilter === f
                ...
                filteredDemandes.map((d, i) => (
                  ...
                  >
                    <div className="absolute top-0 right-0 p-4">
                      <div className={`px-3 py-1 rounded-full border-2 text-[8px] font-black uppercase tracking-widest ${d.statut === 'en_attente' ? 'bg-orange-50 text-orange-600 border-orange-100' :
