...
export function AdminSettings() {
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            <div className="border-b-2 border-slate-100 pb-6">
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Configurez l'identité principale du réseau.</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nom de la plateforme</label>
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email de contact système</label>
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Langue par défaut</label>
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Fuseau Horaire</label>
                <select className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-bold text-slate-900 appearance-none cursor-pointer shadow-inner">
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            <div className="border-b-2 border-slate-100 pb-6">
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Gérez la sécurité des comptes et des sessions.</p>
            </div>
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            <div className="border-b-2 border-slate-100 pb-6">
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Règles d'envoi des messages automatiques.</p>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
            <div className="border-b-2 border-slate-100 pb-6 flex justify-between items-end">
              <div>
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Rétention et intégrité des données.</p>
              </div>
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
             <div className="border-b-2 border-slate-100 pb-6">
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Personnalisation visuelle globale.</p>
            </div>
            <div className="flex items-center justify-between p-6 bg-white border-2 border-slate-100 rounded-3xl shadow-sm hover:border-blue-200 transition-all">
                <div className="flex items-center gap-5">
  ...
  return (
    <div className="animate-fadeIn bg-slate-50 min-h-screen">
      <div className="sticky top-0 z-40 bg-slate-50/80 backdrop-blur-xl px-8 lg:px-12 py-6 border-b border-slate-200/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-8">
        <div>
          <h1 className="text-4xl font-black text-slate-900 uppercase tracking-tight mb-2">Paramètres Système</h1>
          <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Configuration globale et sécurité de l'infrastructure.</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={isSaving}
          className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl active:scale-95 border-2 ${
            isSaving 
              ? 'bg-emerald-500 border-emerald-400 text-white shadow-emerald-200/50' 
              : 'bg-blue-600 border-blue-500 text-white hover:bg-blue-700 hover:shadow-blue-300'
          }`}
        >
          {isSaving ? (
            <>
      ...
      <div className="flex flex-col xl:flex-row gap-10 max-w-7xl">
        {/* Sidebar Navigation */}
        <div className="xl:w-80 flex-shrink-0 space-y-2 bg-white p-4 rounded-[2.5rem] border-2 border-slate-100 shadow-sm h-fit">
          {SETTINGS_TABS.map((tab) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center justify-center ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-blue-500'}`}>
                  <tab.icon className="w-6 h-6" />
                </div>
                <div className="text-left">
                  <p className={`font-black text-sm uppercase tracking-tight leading-none mb-1 ${isActive ? 'text-white' : 'text-slate-700 group-hover:text-blue-600'}`}>{tab.label}</p>
                  <p className={`text-[9px] font-bold uppercase tracking-widest ${isActive ? 'text-blue-100' : 'text-slate-400'}`}>{tab.desc}</p>
                </div>
              </button>
            );
          })}
        ...
        {/* Content Area */}
        <div className="flex-1 bg-white p-8 lg:p-12 rounded-[3rem] border-2 border-slate-100 shadow-2xl shadow-slate-200/50 min-h-[600px] relative overflow-hidden">
