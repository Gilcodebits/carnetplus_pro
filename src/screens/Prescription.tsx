...
export function Prescription() {
    ...
    <div className="p-8 animate-scaleIn flex items-center justify-center min-h-[80vh] bg-slate-200">
      <Card className="max-w-md w-full p-12 rounded-[3rem] border-2 border-slate-200 shadow-2xl shadow-slate-200/50 text-center bg-white">
        ...
        <div className="space-y-4">
          ...
          <button className="w-full py-4 bg-white border-2 border-slate-100 text-slate-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-50 transition-all">
  ...
  return (
    <div className="animate-fadeIn bg-slate-50 min-h-screen w-full max-w-full overflow-x-hidden flex flex-col">
      ...
      <div className="fixed top-0 left-0 lg:left-64 right-0 z-50 bg-white border-b-2 border-slate-200 shadow-md h-[90px] flex items-center shrink-0 no-print">
        <div className="px-6 md:px-10 flex flex-row justify-between items-center w-full gap-4">
          <div className="flex items-center gap-4">
            <div className="w-1.5 h-10 bg-emerald-600 rounded-full shrink-0 shadow-sm shadow-emerald-200" />
            <div>
              <h1 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight leading-none">Nouvelle Ordonnance</h1>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate(-1)}
              className="p-3 bg-white text-slate-400 hover:text-emerald-600 rounded-xl transition-all border-2 border-slate-100 hover:border-emerald-200 shadow-sm active:scale-90"
              title="Retour"
            >
            ...
            <button
              onClick={handleSave}
              disabled={loading}
              className="flex items-center justify-center gap-3 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition-all active:scale-95 shadow-lg shadow-emerald-200 disabled:opacity-50"
            >
      ...
      <div className="flex-1 px-4 md:px-10 pb-12 pt-[130px] md:pt-[140px]">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Sidebar: Récapitulatif - Stacked on top for mobile review */}
          <div className="lg:col-span-4 space-y-6 order-2 lg:order-1">
            <Card className="rounded-[2rem] md:rounded-[3rem] border-2 border-slate-200 shadow-2xl shadow-slate-200/50 p-6 md:p-8 bg-white h-fit lg:sticky lg:top-10">
              <h3 className="text-lg md:text-xl font-black text-slate-900 mb-6 md:mb-8 flex items-center gap-4 uppercase tracking-tight">
                <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center border border-emerald-100 shadow-sm shrink-0">
              ...
              {meds.length === 0 ? (
                <div className="py-12 text-center border-2 border-dashed border-slate-100 rounded-[2rem] bg-slate-50/50">
                  ...
                  <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest italic px-6">Aucun médicament ajouté pour le moment</p>
                </div>
              ) : (
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 scrollbar-hide">
                  {meds.map((m, idx) => (
                    <div key={m.id}
                      onClick={() => editMed(m)}
                      className={`p-4 md:p-5 border-2 transition-all cursor-pointer group rounded-[1.5rem] md:rounded-[1.8rem] animate-slideInRight ${currentMed.id === m.id ? 'border-emerald-500 bg-emerald-50' : 'bg-slate-50 border-slate-100 hover:border-emerald-200 hover:bg-white'}`}>
                      <div className="flex justify-between items-start gap-4 mb-2">
                        <p className="font-black text-slate-900 text-sm uppercase leading-tight">{m.nom}</p>
                        <button onClick={(e) => { e.stopPropagation(); removeMed(m.id); }} className="text-slate-500 hover:text-rose-500 transition-colors">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <span className="text-[9px] text-slate-600 font-black uppercase tracking-tighter truncate">{m.instructions || ""}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-10 space-y-4">
                ...
                <button onClick={() => navigate(`/medecin/dossier/${id}`)}
                  className="w-full py-4 bg-white text-slate-600 border-2 border-slate-100 rounded-[2rem] font-black text-[9px] uppercase tracking-widest hover:bg-slate-50 hover:text-slate-900 transition-all"
                >
                  Tout annuler
                </button>
              </div>
          ...
          <div className="lg:col-span-8 space-y-6 md:space-y-8 order-1 lg:order-2">
            ...
            <div className={`transition-all duration-500 ${isAdding ? 'opacity-0 scale-95 translate-x-[-100px]' : 'opacity-100 scale-100 translate-x-0'}`}>
              <Card className="rounded-[2.5rem] md:rounded-[4rem] border-2 border-slate-200 shadow-2xl shadow-slate-200/50 p-6 md:p-12 bg-white relative overflow-hidden group">
                {/* Background Accent */}
                ...
                <div className="flex items-center justify-between mb-12 relative z-10">
                  <div className="flex items-center gap-4 md:gap-6">
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-emerald-50 border-2 border-emerald-100 rounded-xl md:rounded-[1.8rem] flex items-center justify-center shadow-sm shrink-0">
                      <Pill className="w-6 h-6 md:w-8 md:h-8 text-emerald-600" />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-3">
                        <span className="hidden md:block w-6 h-1.5 bg-emerald-500 rounded-full" />
                        <h3 className="text-xl md:text-3xl font-black text-slate-900 uppercase tracking-tight truncate">
                          {meds.find(m => m.id === currentMed.id) ? "Modification" : "Nouveau Médicament"}
                        </h3>
                      </div>
                      <p className="text-[9px] md:text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1 md:ml-9">Élément de prescription actif</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <input
                      ...
                      className="w-full px-6 md:px-8 py-5 md:py-6 border-2 border-slate-100 rounded-[1.5rem] md:rounded-[2rem] bg-slate-50 focus:bg-white focus:outline-none focus:ring-8 focus:ring-emerald-500/5 focus:border-emerald-500 text-sm md:text-base font-bold transition-all placeholder-slate-300 shadow-inner"
                    />
                  </div>
                  <div className="space-y-4">
                  ...
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <textarea
                      ...
                      className="w-full px-6 md:px-8 py-5 md:py-6 border-2 border-slate-100 rounded-[1.5rem] md:rounded-[2rem] bg-slate-50 focus:bg-white focus:outline-none focus:ring-8 focus:ring-emerald-500/5 focus:border-emerald-500 text-sm md:text-base font-bold transition-all placeholder-slate-300 shadow-inner resize-none"
                    />
                  </div>
                </div>
                ...
                <div className="mt-8 md:mt-12 flex justify-end">
                  <button
                    ...
                    className="w-full md:w-auto group flex items-center justify-center gap-4 md:gap-6 px-10 md:px-14 py-5 md:py-7 bg-emerald-600 text-white rounded-2xl md:rounded-[2.5rem] font-black text-[10px] md:text-xs uppercase tracking-widest hover:shadow-2xl hover:shadow-emerald-300 hover:-translate-y-1 transition-all shadow-xl shadow-emerald-500/20 active:scale-95 border-2 border-emerald-400"
                  >
                    <Plus className="w-6 h-6 md:w-7 md:h-7 group-hover:rotate-90 transition-transform duration-500" />
                    {meds.find(m => m.id === currentMed.id) ? "METTRE À JOUR" : "AJOUTER"}
                  </button>
                </div>
              </Card>

              <div className="mt-8 md:mt-12 p-6 md:p-10 bg-blue-50/50 border-2 border-blue-100 border-dashed rounded-[2.5rem] md:rounded-[3rem] flex items-center gap-6 md:gap-8 group">
                <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-xl md:rounded-2xl flex items-center justify-center border-2 border-blue-200 shadow-sm text-blue-500 group-hover:scale-110 transition-transform shrink-0">
                  <Info className="w-6 h-6 md:w-8 md:h-8" />
                </div>
                <div>
                  <p className="text-[9px] md:text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Aide au praticien</p>
                  <p className="text-xs md:text-sm font-bold text-blue-700 leading-relaxed max-w-xl">Une fois vos médicaments ajoutés à la liste de gauche, cliquez sur le bouton <span className="font-black uppercase tracking-tighter">Valider Tout</span> pour enregistrer définitivement la prescription.</p>
                </div>
              </div>
            </div>
          </div>
